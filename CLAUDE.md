...
- Identifiants de clic (`fbclid`, `gclid`, `gbraid`, `wbraid`) capturés depuis l'URL, persistés en localStorage (`hy_<id>`), enregistrés en colonnes et poussés dans `quiz_complete`. Seul fbclid est repassé dans l'URL de redirection. La LP Systeme.io doit transmettre ces paramètres au lien vers le quiz, sinon rien n'est capté. `gclid` sert à l'import de conversions hors ligne Google Ads au moment du prélèvement J8 (fenêtre 90 jours).
...
- GTM (conteneur GTM-TGSR3C3K posé dans `poids/index.html`) et pushes dataLayer par le moteur : `start_quiz` à l'arrivée sur la question 2, puis `q3`, `q4`... `q16` à chaque question, et `quiz_complete` à la soumission avec email, telephone (E.164) et prenom. Chaque événement porte aussi `quiz_type` et `session_id`, et ne part qu'une fois (le bouton Retour ne recrée pas de doublon).
- Le formulaire final demande prénom, email et téléphone (obligatoire, normalisé en E.164 par `normalizeTel` pour les correspondances avancées Meta/Google, colonne `telephone`). Consentement email et SMS.
- Après la capture email : écran de processing intégré au quiz (fond sombre, anneau de progression ~7,5 s, étapes `QUIZ.processingSteps` qui se cochent), puis redirection vers redirect_url_results avec uniquement : sid, prenom, fbclid. La page de résultats récupère les réponses via la RPC `quiz_get(p_id uuid)` (security definer, renvoie email + champs de personnalisation en jsonb, jamais le tracking) pour personnaliser la page et pousser l'email en dataLayer. La même URL est stockée en colonne `redirect_url` et part dans le webhook.
